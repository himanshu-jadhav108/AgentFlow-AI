...
class RetrievedChunk(BaseModel):
    ...
    score: float = Field(
        ...,
        description="Raw similarity score from vector store (e.g. FAISS L2/Cosine).",
    )
...
class IndexResponse(BaseModel):
    ...
    status: str = Field(
        ..., description="Status of the index rebuild (e.g., 'success')."
    )
    documents_processed: int = Field(
        ..., description="Number of source documents loaded and cleaned."
    )
    chunks_created: int = Field(
        ..., description="Number of vector chunks generated and indexed."
    )
...
class GraphRunResponse(BaseModel):
    ...
    classification: str = Field(
        ...,
        description="The final classification category determined by the agent triage.",
    )
    node_path: List[str] = Field(
        ..., description="The ordered sequence of graph nodes executed in this run."
    )
    final_state: Dict[str, Any] = Field(
        ..., description="Full state snapshot at the conclusion of the execution path."
    )
